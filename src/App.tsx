...
function App() {
  ...
  const [buildingnames, setBuilding] = useState<any>(defaultName);

  const updateBuildingName = (newBuildingName: any) => {
    setBuilding(newBuildingName);
  };
  ...
  return (
    <>
      {loggedInState === true ? (
        <div>
          <CalciteShell>
            <MyContext value={{ buildingnames, updateBuildingName }}>
              <ActionPanel />
              <UndergroundSwitch />
              <MapDisplay />
              <Chart />
